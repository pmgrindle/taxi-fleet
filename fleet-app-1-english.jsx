...
const FONT_URL = "https://fonts.googleapis.com/css2?family=Inter:wght@400;600;700;900&display=swap";

const INIT_DRIVERS = [
  { id: 1, name: "Abdul Hakeem", car: "37963", dailyRequired: 28, active: true, balance: -4,  phone: "0791000001", username: "abdulhakeem", password: "1234" },
  { id: 2, name: "Anwar",        car: "45821", dailyRequired: 32, active: true, balance: 0,   phone: "0791000002", username: "anwar",        password: "1234" },
  { id: 3, name: "Badr Masa'ada",  car: "62104", dailyRequired: 28, active: true, balance: 8,   phone: "0791000003", username: "badr",          password: "1234" },
  { id: 4, name: "Sami Al-Khalid", car: "71345", dailyRequired: 35, active: true, balance: -12, phone: "0791000004", username: "sami",          password: "1234" },
  { id: 5, name: "Mahmoud Al-Nour",  car: "88901", dailyRequired: 30, active: true, balance: 0,   phone: "0791000005", username: "mahmoud",       password: "1234" },
];

const INIT_PAYMENTS = [
  { id: 1, driverId: 1, driverName: "Abdul Hakeem", car: "37963", date: "2026-03-07", expected: 28, paid: 24, diff: -4,  method: "Cash",        note: "Partial payment", status: "partial" },
  { id: 2, driverId: 2, driverName: "Anwar",        car: "45821", date: "2026-03-07", expected: 32, paid: 32, diff: 0,   method: "CliQ",        note: "",        status: "full"    },
  { id: 3, driverId: 3, driverName: "Badr Masa'ada",  car: "62104", date: "2026-03-07", expected: 28, paid: 36, diff: 8,   method: "Zain Cash",    note: "Overpayment",   status: "over"    },
  { id: 4, driverId: 4, driverName: "Sami Al-Khalid", car: "71345", date: "2026-03-07", expected: 35, paid: 0,  diff: -35, method: "",            note: "",        status: "unpaid"  },
  { id: 5, driverId: 5, driverName: "Mahmoud Al-Nour",  car: "88901", date: "2026-03-07", expected: 30, paid: 30, diff: 0,   method: "Bank Transfer", note: "",        status: "full"    },
  { id: 6, driverId: 1, driverName: "Abdul Hakeem",  car: "37963", date: "2026-03-06", expected: 28, paid: 28, diff: 0,   method: "Cash",        note: "",        status: "full"    },
  { id: 7, driverId: 2, driverName: "Anwar",         car: "45821", date: "2026-03-06", expected: 32, paid: 28, diff: -4,  method: "CliQ",        note: "",        status: "partial" },
];
...
const INIT_EXPENSES = [
  ...
  { id: 3, date: "2026-03-05", category: "Fuel",       description: "Reserve gasoline",           amount: 25  },
];

const TODAY = "2026-03-11";

const statusColor = (s) => ({ full: "#22c55e", partial: "#f59e0b", over: "#3b82f6", unpaid: "#ef4444" }[s] || "#ef4444");
const statusLabel = (s) => ({ full: "Paid", partial: "Partial", over: "Over", unpaid: "Unpaid" }[s] || "Unpaid");
const fmtDiff = (n) => (n >= 0 ? "+" : "") + n + " JD";

const S = {
  input: { width: "100%", background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.18)", borderRadius: 10, padding: "11px 14px", color: "#f1f5f9", fontSize: 14, outline: "none", boxSizing: "border-box", fontFamily: "Inter, sans-serif" },
...
export default function App() {
  ...
  function doLogin(username, password) {
    if (username === "admin" && password === "admin123") {
      setUser({ role: "admin", name: "Admin" }); return true;
    }
  ...
  const root = { minHeight: "100vh", background: "#0f172a", fontFamily: "Inter, sans-serif", color: "#f1f5f9" };
...
function LoginPage({ onLogin, drivers }) {
  ...
  function go() { if (!onLogin(u.trim(), p)) setErr("Incorrect username or password"); }
    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "linear-gradient(135deg,#0f172a,#1e293b,#0f172a)" }}>
      <div style={{ width: 400, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 24, padding: 40 }}>
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          ...
          <h2 style={{ color: "#f8fafc", margin: "8px 0 4px", fontWeight: 900 }}>Jordan Fleet</h2>
          <p style={{ color: "#94a3b8", margin: 0, fontSize: 14 }}>Driver Payment Management System</p>
        </div>
        ...
        <button onClick={go} style={{ width: "100%", background: "linear-gradient(135deg,#f59e0b,#d97706)", border: "none", borderRadius: 12, padding: 14, color: "#1c1917", fontWeight: 900, fontSize: 16, cursor: "pointer", fontFamily: "Inter, sans-serif" }}>
          Log In
        </button>
    ...
    <div style={{ display: "flex", minHeight: "100vh" }}>
      <div style={{ width: 210, background: "rgba(255,255,255,0.03)", borderLeft: "1px solid rgba(255,255,255,0.07)", display: "flex", flexDirection: "column", flexShrink: 0 }}>
        <div style={{ padding: "24px 20px 20px", textAlign: "center", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
          ...
          <div style={{ color: "#f59e0b", fontWeight: 900, fontSize: 15 }}>Jordan Fleet</div>
          <div style={{ color: "#475569", fontSize: 12 }}>Admin Panel</div>
        </div>
        <nav style={{ flex: 1, paddingTop: 12 }}>
          {nav.map(n => (
            <button key={n.id} onClick={() => setPage(n.id)}
              style={{ width: "100%", display: "flex", alignItems: "center", gap: 10, padding: "11px 20px", background: page === n.id ? "rgba(245,158,11,0.15)" : "transparent", border: "none", borderRight: page === n.id ? "3px solid #f59e0b" : "3px solid transparent", color: page === n.id ? "#f59e0b" : "#94a3b8", cursor: "pointer", fontSize: 14, fontFamily: "Inter, sans-serif", fontWeight: 600 }}>
              {n.icon} {n.label}
            </button>
          ))}
        </nav>
        <div style={{ padding: "16px 20px" }}>
          <button onClick={logout} style={{ width: "100%", background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 10, padding: 10, color: "#f87171", cursor: "pointer", fontFamily: "Inter, sans-serif", fontWeight: 700, fontSize: 13 }}>🚪 Logout</button>
        </div>
      </div>
...
function Dashboard({ drivers, payments, expenses }) {
  ...
  return (
    <div style={{ padding: 32 }}>
      <h2 style={{ color: "#f8fafc", fontWeight: 900, margin: "0 0 4px" }}>Dashboard</h2>
      <p style={{ color: "#64748b", margin: "0 0 24px", fontSize: 13 }}>Today: {TODAY}</p>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginBottom: 28 }}>
        {[
          { label: "Expected Today",    val: totExp + " JD",              icon: "🎯", c: "#3b82f6" },
          { label: "Collected Today",    val: totColl + " JD",             icon: "✅", c: "#22c55e" },
          { label: "Deficit",             val: Math.abs(shortage) + " JD",  icon: "⚠️", c: "#f59e0b" },
          { label: "Today's Net",        val: (totColl - expToday) + " JD",icon: "💎", c: "#a78bfa" },
        ].map((s, i) => (
      ...
      <h3 style={{ color: "#f1f5f9", fontWeight: 700, marginBottom: 14, fontSize: 16 }}>Driver Status Today</h3>
        ...
        {drivers.map(d => {
          ...
          return (
            <div key={d.id} style={{ background: "rgba(255,255,255,0.04)", border: `1px solid ${statusColor(st)}35`, borderRadius: 12, padding: 14 }}>
              ...
              {p
                ? <div style={{ color: "#94a3b8", fontSize: 12, marginTop: 6 }}>Paid <strong style={{ color: statusColor(st) }}>{p.paid}</strong> of {p.expected} JD</div>
                : <div style={{ color: "#ef4444", fontSize: 12, marginTop: 6 }}>Not paid yet</div>}
            </div>
          );
        })}
      </div>
...
function Payments({ payments, setPayments, showToast }) {
  ...
  return (
    <div style={{ padding: 32 }}>
      <h2 style={{ color: "#f8fafc", fontWeight: 900, marginBottom: 20 }}>Payment Records</h2>
      <div style={{ display: "flex", gap: 8, marginBottom: 18, flexWrap: "wrap", alignItems: "center" }}>
        {["all", "full", "partial", "over", "unpaid"].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            style={{ padding: "7px 16px", borderRadius: 20, border: "none", cursor: "pointer", fontFamily: "Inter, sans-serif", fontWeight: 600, fontSize: 12, background: filter === f ? "#f59e0b" : "rgba(255,255,255,0.07)", color: filter === f ? "#1c1917" : "#94a3b8" }}>
      ...
      <div style={{ background: "rgba(255,255,255,0.03)", borderRadius: 14, overflow: "hidden", border: "1px solid rgba(255,255,255,0.07)" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead><tr style={{ background: "rgba(255,255,255,0.05)" }}>
            {["Date", "Driver", "Car", "Expected", "Paid", "Difference", "Method", "Status", ""].map((h, i) => <th key={i} style={thStyle}>{h}</th>)}
          </tr></thead>
          <tbody>
            {list.map(p => (
              <tr key={p.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                ...
                <td style={tdStyle}>
                  <button onClick={() => { setPayments(v => v.filter(x => x.id !== p.id)); showToast("Deleted"); }}
                    style={{ background: "rgba(239,68,68,0.1)", border: "none", color: "#f87171", cursor: "pointer", borderRadius: 8, padding: "5px 10px", fontSize: 12, fontFamily: "Inter, sans-serif" }}>Delete</button>
                </td>
              </tr>
            ))}
            {list.length === 0 && <tr><td colSpan={9} style={{ padding: 36, textAlign: "center", color: "#475569" }}>No records found</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
...
function Drivers({ drivers, setDrivers, payments, showToast }) {
  ...
  function validate() {
    if (!form.name.trim())     return "Please enter driver name";
    if (!form.car.trim())      return "Please enter car number";
  ...
  return (
    <div style={{ padding: 32 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
        <div>
          <h2 style={{ color: "#f8fafc", fontWeight: 900, margin: 0 }}>Driver Management</h2>
          <p style={{ color: "#64748b", margin: "4px 0 0", fontSize: 13 }}>{drivers.length} drivers — {drivers.filter(d => d.active).length} active</p>
        </div>
        <button onClick={() => { setForm(EMPTY); setModal({ mode: "add" }); }}
          style={{ background: "linear-gradient(135deg,#f59e0b,#d97706)", border: "none", borderRadius: 12, padding: "10px 22px", color: "#1c1917", fontWeight: 900, fontSize: 14, cursor: "pointer", fontFamily: "Inter, sans-serif" }}>
          + Add Driver
        </button>
      </div>
          ...
          <div key={d.id} style={{ ...S.card, border: `1px solid ${d.active ? "rgba(34,197,94,0.2)" : "rgba(255,255,255,0.07)"}` }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 12 }}>
              <div style={{ background: "rgba(255,255,255,0.04)", borderRadius: 10, padding: "8px 12px" }}>
                <div style={{ color: "#64748b", fontSize: 11 }}>Daily Amount</div>
            ...
            <div style={{ background: "rgba(167,139,250,0.08)", border: "1px solid rgba(167,139,250,0.2)", borderRadius: 10, padding: "10px 14px", marginBottom: 12 }}>
              <div style={{ color: "#94a3b8", fontSize: 11, marginBottom: 5, fontWeight: 700 }}>🔐 Login Credentials</div>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => { setForm({ ...d }); setModal({ mode: "edit", id: d.id }); }}
                style={{ flex: 1, background: "rgba(245,158,11,0.1)", border: "1px solid rgba(245,158,11,0.2)", borderRadius: 10, padding: 8, color: "#f59e0b", cursor: "pointer", fontSize: 13, fontFamily: "Inter, sans-serif", fontWeight: 600 }}>✏️ Edit</button>
              <button onClick={() => setDrivers(p => p.map(x => x.id === d.id ? { ...x, active: !x.active } : x))}
                style={{ flex: 1, background: "rgba(255,255,255,0.05)", border: "none", borderRadius: 10, padding: 8, color: "#94a3b8", cursor: "pointer", fontSize: 13, fontFamily: "Inter, sans-serif" }}>
                {d.active ? "⏸ Pause" : "▶ Activate"}
              </button>
              <button onClick={() => { if (payments.some(p => p.driverId === d.id)) return showToast("Cannot delete a driver with existing records", "error"); setDrivers(p => p.filter(x => x.id !== d.id)); showToast("Deleted"); }}
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: 16 }}>
          <div style={{ background: "#1e293b", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 20, padding: 28, width: "100%", maxWidth: 520, maxHeight: "90vh", overflowY: "auto" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 22 }}>
              <h3 style={{ color: "#f59e0b", margin: 0, fontSize: 17, fontWeight: 900 }}>{modal.mode === "add" ? "➕ Add Driver" : "✏️ Edit Driver"}</h3>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 16px" }}>
              <Field label="Driver Name *" value={form.name} onChange={v => setF("name", v)} placeholder="Full name" span={2} />
              <Field label="Car Number *" value={form.car} onChange={v => setF("car", v)} placeholder="37963" />
              <Field label="Daily Amount (JD)" value={form.dailyRequired} onChange={v => setF("dailyRequired", v)} type="number" placeholder="28" />
              <Field label="Phone Number" value={form.phone} onChange={v => setF("phone", v)} placeholder="07xxxxxxxx" span={2} />
            </div>
            <div style={{ borderTop: "1px solid rgba(255,255,255,0.08)", margin: "16px 0", paddingTop: 16 }}>
              <p style={{ color: "#a78bfa", fontSize: 13, fontWeight: 700, margin: "0 0 12px" }}>🔐 Login Credentials</p>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 16px" }}>
                <Field label="Username *" value={form.username} onChange={v => setF("username", v.toLowerCase().replace(/\s/g, ""))} placeholder="ahmad2026" />
                <Field label="Password *" value={form.password} onChange={v => setF("password", v)} placeholder="Password" />
              </div>
              <p style={{ background: "rgba(167,139,250,0.08)", borderRadius: 8, padding: "9px 12px", fontSize: 12, color: "#64748b", margin: "4px 0 0" }}>
                💡 The driver uses these credentials to log in and record their daily payments
              </p>
            </div>
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={save} style={{ flex: 1, background: "linear-gradient(135deg,#f59e0b,#d97706)", border: "none", borderRadius: 12, padding: 13, color: "#1c1917", fontWeight: 900, fontSize: 15, cursor: "pointer", fontFamily: "Inter, sans-serif" }}>
                {modal.mode === "add" ? "✅ Add" : "💾 Save"}
              </button>
              <button onClick={() => setModal(null)} style={{ flex: 1, background: "rgba(255,255,255,0.07)", border: "none", borderRadius: 12, padding: 13, color: "#94a3b8", fontWeight: 700, cursor: "pointer", fontFamily: "Inter, sans-serif" }}>Cancel</button>
            </div>
          </div>
        </div>
      )}
...
function Expenses({ expenses, setExpenses, showToast }) {
  ...
  function add() {
    ...
    setForm({ date: TODAY, category: "Maintenance", description: "", amount: "" });
    setShow(false); showToast("Expense added");
  }
  ...
  return (
    <div style={{ padding: 32 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 22 }}>
        <div>
          <h2 style={{ color: "#f8fafc", fontWeight: 900, margin: 0 }}>Expenses</h2>
          <p style={{ color: "#64748b", margin: "4px 0 0", fontSize: 13 }}>Total: <strong style={{ color: "#f87171" }}>{total} JD</strong></p>
        </div>
        <button onClick={() => setShow(true)} style={{ background: "linear-gradient(135deg,#f59e0b,#d97706)", border: "none", borderRadius: 12, padding: "10px 22px", color: "#1c1917", fontWeight: 900, fontSize: 14, cursor: "pointer", fontFamily: "Inter, sans-serif" }}>+ Add</button>
      </div>
      {show && (
        <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(245,158,11,0.25)", borderRadius: 14, padding: 22, marginBottom: 22 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: "0 14px" }}>
            <Field label="Date" value={form.date} onChange={v => setF("date", v)} type="date" />
            <Field label="Category" value={form.category} onChange={v => setF("category", v)} options={cats} />
            <Field label="Description" value={form.description} onChange={v => setF("description", v)} placeholder="Expense description" />
            <Field label="Amount (JD)" value={form.amount} onChange={v => setF("amount", v)} type="number" placeholder="0" />
          </div>
          <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
            <button onClick={add} style={{ background: "#f59e0b", border: "none", borderRadius: 10, padding: "9px 22px", color: "#1c1917", fontWeight: 700, cursor: "pointer", fontFamily: "Inter, sans-serif" }}>Save</button>
            <button onClick={() => setShow(false)} style={{ background: "rgba(255,255,255,0.07)", border: "none", borderRadius: 10, padding: "9px 22px", color: "#94a3b8", cursor: "pointer", fontFamily: "Inter, sans-serif" }}>Cancel</button>
          </div>
        </div>
      )}
      <div style={{ background: "rgba(255,255,255,0.03)", borderRadius: 14, overflow: "hidden", border: "1px solid rgba(255,255,255,0.07)" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead><tr style={{ background: "rgba(255,255,255,0.05)" }}>
            {["Date", "Category", "Description", "Amount", ""].map((h, i) => <th key={i} style={thStyle}>{h}</th>)}
          </tr></thead>
          <tbody>
            {[...expenses].sort((a, b) => b.date.localeCompare(a.date)).map(e => (
              <tr key={e.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                ...
                <td style={{ padding: "11px 14px" }}>
                  <button onClick={() => { setExpenses(p => p.filter(x => x.id !== e.id)); showToast("Deleted"); }}
                    style={{ background: "rgba(239,68,68,0.1)", border: "none", color: "#f87171", cursor: "pointer", borderRadius: 8, padding: "5px 10px", fontSize: 12, fontFamily: "Inter, sans-serif" }}>Delete</button>
                </td>
              </tr>
            ))}
...
function Reports({ drivers, payments, expenses }) {
  ...
  return (
    <div style={{ padding: 32 }}>
      <h2 style={{ color: "#f8fafc", fontWeight: 900, marginBottom: 22 }}>Financial Reports</h2>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginBottom: 28 }}>
        {[
          { label: "Total Collected",  val: tc + " JD",  c: "#22c55e" },
          { label: "Total Expenses", val: tx + " JD",  c: "#f87171" },
          { label: "Net Profit",      val: (tc-tx)+" JD",c: "#a78bfa" },
          { label: "Collection Rate",    val: rate + "%",  c: "#f59e0b" },
        ].map((s, i) => (
      ...
      <h3 style={{ color: "#f1f5f9", fontWeight: 700, marginBottom: 14, fontSize: 16 }}>Driver Performance</h3>
      <div style={{ background: "rgba(255,255,255,0.03)", borderRadius: 14, overflow: "hidden", border: "1px solid rgba(255,255,255,0.07)" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead><tr style={{ background: "rgba(255,255,255,0.05)" }}>
            {["Driver", "Car", "Days", "Expected", "Collected", "Difference", "Performance"].map((h, i) => <th key={i} style={thStyle}>{h}</th>)}
          </tr></thead>
          <tbody>
            {ds.map(d => {
...
function DriverShell({ user, drivers, payments, setPayments, logout, showToast }) {
  ...
  function submit() {
    if (!form.amount) return showToast("Please enter the amount", "error");
    if (paidToday)    return showToast("Payment already recorded for today", "error");
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", maxWidth: 480, margin: "0 auto" }}>
      <div style={{ background: "rgba(245,158,11,0.1)", borderBottom: "1px solid rgba(245,158,11,0.2)", padding: "14px 18px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          <div style={{ color: "#f59e0b", fontWeight: 900, fontSize: 16 }}>Hello, {driver.name}</div>
          <div style={{ color: "#64748b", fontSize: 12 }}>🚗 {driver.car} · 👤 {driver.username}</div>
        </div>
        <button onClick={logout} style={{ background: "rgba(239,68,68,0.1)", border: "none", color: "#f87171", cursor: "pointer", borderRadius: 10, padding: "7px 14px", fontSize: 13, fontFamily: "Inter, sans-serif" }}>Logout</button>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, padding: "14px 14px 0" }}>
        {[
          { val: totalPaid + " JD", label: "Total Paid", c: "#22c55e" },
          { val: driver.dailyRequired + " JD", label: "Daily Required", c: "#f59e0b" },
          { val: fmtDiff(balance), label: "Balance", c: balance < 0 ? "#ef4444" : "#22c55e" },
        ].map((s, i) => (
      ...
      <div style={{ display: "flex", padding: "12px 14px 0" }}>
        {[{ id: "submit", label: "Submit Payment" }, { id: "history", label: "History" }].map(t => (
          <button key={t.id} onClick={() => setTab(t.id)}
            style={{ flex: 1, padding: 12, border: "none", borderBottom: tab === t.id ? "2px solid #f59e0b" : "2px solid transparent", background: "transparent", color: tab === t.id ? "#f59e0b" : "#64748b", fontFamily: "Inter, sans-serif", fontWeight: 700, fontSize: 14, cursor: "pointer" }}>
            {t.label}
          </button>
        ))}
      </div>
      <div style={{ flex: 1, padding: 14 }}>
          ...
          <div style={{ ...S.card, border: "1px solid rgba(255,255,255,0.07)" }}>
            {paidToday && (
              <div style={{ background: "rgba(245,158,11,0.1)", border: "1px solid rgba(245,158,11,0.3)", borderRadius: 10, padding: 12, marginBottom: 14, color: "#f59e0b", fontSize: 13 }}>
                ⚠️ You already submitted a payment for today: {paidToday.paid} JD
              </div>
            )}
            ...
            <div style={{ marginBottom: 14 }}>
              <label style={S.label}>Amount Paid (JD) — Required: {driver.dailyRequired} JD</label>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                {["Cash", "CliQ", "Zain Cash", "Bank Transfer"].map(m => (
                  <button key={m} onClick={() => setForm(p => ({ ...p, method: m }))}
                    style={{ padding: 10, borderRadius: 10, border: form.method === m ? "1px solid #f59e0b" : "1px solid rgba(255,255,255,0.1)", background: form.method === m ? "rgba(245,158,11,0.15)" : "rgba(255,255,255,0.04)", color: form.method === m ? "#f59e0b" : "#94a3b8", cursor: "pointer", fontFamily: "Inter, sans-serif", fontWeight: 600, fontSize: 13 }}>
                    {m}
                  </button>
                ))}
              </div>
            ...
            <div style={{ marginBottom: 18 }}>
              ...
              <textarea value={form.note} onChange={e => setForm(p => ({ ...p, note: e.target.value }))} rows={2} placeholder="Any additional notes..."
                style={{ ...S.input, resize: "none" }} />
            </div>
            <button onClick={submit} style={{ width: "100%", background: "linear-gradient(135deg,#f59e0b,#d97706)", border: "none", borderRadius: 12, padding: 14, color: "#1c1917", fontWeight: 900, fontSize: 16, cursor: "pointer", fontFamily: "Inter, sans-serif" }}>
              ✅ Submit Payment
            </button>
          </div>
        )}
        {tab === "history" && (
          <div>
            {mine.length === 0 && <div style={{ color: "#475569", textAlign: "center", padding: 40 }}>No records yet</div>}
            {mine.map(p => (
              <div key={p.id} style={{ background: "rgba(255,255,255,0.04)", border: `1px solid ${statusColor(p.status)}30`, borderRadius: 12, padding: 14, marginBottom: 10 }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  <span style={{ color: "#64748b", fontSize: 12 }}>Paid: <strong style={{ color: "#f1f5f9" }}>{p.paid} JD</strong></span>
                  <span style={{ color: "#64748b", fontSize: 12 }}>Difference: <strong style={{ color: statusColor(p.status) }}>{fmtDiff(p.diff)}</strong></span>
